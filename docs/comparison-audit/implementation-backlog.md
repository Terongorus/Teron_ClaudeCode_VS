...
| **FEAT-2** | ✅ done | E | Native VS diff tab, auto-opened on an edit prompt. Accept/revert stay on the card — see below. **New caveat (Phase 20, 2026-09-02):** when the proposed edit's line endings don't match the original file's, opening the tab can trigger VS's own native "Inconsistent Line Endings" dialog (once per temp file — two in the case observed). That dialog is real Win32 modal chrome, so it correctly steals OS-level keyboard focus off `InputBox`, and nothing hands focus back once it's dismissed — the 1/2/3 permission shortcuts go quiet until the user clicks the composer again. Not a regression of 86caecd's focus-return fix (that fix's own precondition — a fresh card while focus sits on an unrelated *WPF* control — was independently live-confirmed working in the same pass); this is a new, separate rough edge in the diff-tab flow. Not fixed here per this pass's scope (verification/documentation only) — see [Phase 20](../../Phase%2020%20-%20Post-Fix%20Live%20Re-Verification.md). |
